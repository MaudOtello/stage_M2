...
// Dirichlet_multinomial d'après Smith et al, 2022
functions {
...
generated quantities {
  // Prédiction totale
  array[N_pred, K_pred] vector[S] Y_pred ; 
  for (n in 1:N_pred){
  for (k in 1:K_pred){
      Y_pred[n,k]= softmax(alpha + beta*X_pred_cr[n] + gamma*(X_pred_cr[n])^2 + delta*Z_pred_cr[k]+ epsilon*(Z_pred_cr[k])^2); // prédiction de Y quand Z varie (ST)
    }
  }
}
